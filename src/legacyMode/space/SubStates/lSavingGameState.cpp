...
LSavingGameState::LSavingGameState(Urho3D::Context * context, UIElement* uiSubState) :
	ISubState(context, uiSubState)
{
}

void LSavingGameState::Create(Node* targetNode)
{
...
void LSavingGameState::Enter(Node* targetNode)
{
	...
	SubscribeToEvent(fileSelector_->GetOKButton(), E_PRESSED, URHO3D_HANDLER(LSavingGameState, HandleSaveClick));
	SubscribeToEvent(fileSelector_->GetCancelButton(), E_PRESSED, URHO3D_HANDLER(LSavingGameState, HandleBackClick));
}

void LSavingGameState::Exit()
{
...
void LSavingGameState::HandleBackClick(StringHash eventType, VariantMap & eventData)
{
	Exit();
	parentState_->ExitSubstate();
}

void LSavingGameState::HandleSaveClick(StringHash eventType, VariantMap & eventData)
{
