...
class LFindObjectsHelper : public Object
{
	URHO3D_OBJECT(LFindObjectsHelper, Object);

public:
	LFindObjectsHelper(Context* context, Node* controlNode, UIElement* uiSpaceState);
	~LFindObjectsHelper();
...
template<typename T>
inline void LFindObjectsHelper::SetOffShipFromObject(F32 operabilityRadius, Vector2 mousePos, F32 mouseHoveringArea)
{
	if (!controlNode_)
		return;
...
template<typename T>
inline void LFindObjectsHelper::FindClosestObject(Node* systemNode, Vector2 mousePos, F32 visibilityRadius, F32 operabilityRadius, F32 mouseHoveringArea)
{
