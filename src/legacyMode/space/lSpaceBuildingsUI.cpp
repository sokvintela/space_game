...
LSpaceBuildings::LSpaceBuildings(Context* context, Node* controlNode, UIElement* uiSpaceState)
	: Object(context)
...
LSpaceBuildings::~LSpaceBuildings()
{
}

void LSpaceBuildings::UpdateBuildingsUI(Node* enteredNode)
{
	UpdateProposalBuildingsUI(enteredNode);
	UpdateCompletedBuildingsUI(enteredNode);
}

void LSpaceBuildings::UpdateProposalBuildingsUI(Node* enteredNode)
{
	// buildings
	if (!enteredNode)
		return;
	...
	for (U32 i = 0; i < 10; i++)
	{
		...
		for (U32 j = 0; j < 4; j++)
		{
			...
			LBuilding::TargetType tType = LGeneralMethods::GetObjectTargetType(enteredNode_);

			auto buildingPrefab = prefabs->GetBuilding(tType, buildingName);

			Button* proposalBuildButton = buildingsRow->CreateChild<Button>();
			proposalBuildButton->SetTexture(LGeneralMethods::GetTextureFromNode(buildingPrefab));
			...
			LGeneralMethods::ShowCost(proposalBuildButton, controlNode_, cost.first_, cost.second_);

			SubscribeToEvent(proposalBuildButton, E_PRESSED, URHO3D_HANDLER(LSpaceBuildings, HandleObjectBuild));
...
void LSpaceBuildings::UpdateCompletedBuildingsUI(Node* enteredNode)
{
	// buildings
	if (!enteredNode)
		return;
	...
	for (U32 i = 0; i < 12; i++)
	{
		...
		for (U32 j = 0; j < 4; j++)
		{
			...
			completedBuildButton->SetTexture(LGeneralMethods::GetTextureFromNode(completedBuildings[count]));
			...
			LPlanet* planet = enteredNode_->GetComponent<LPlanet>();
			if (planet && planet->GetPlanetType() == LPlanet::PlanetType::PT_Solid)
				SubscribeToEvent(completedBuildButton, E_PRESSED, URHO3D_HANDLER(LSpaceBuildings, HandlePlanetOpen));
...
void LSpaceBuildings::SwitchOff()
{
	proposalBuildingsWindow_->SetVisible(false);
}

void LSpaceBuildings::HandleObjectBuild(StringHash eventType, VariantMap & eventData)
{
	// TODO: distance
	...
	buildEventData[ObjectBuildEvent::TARGET_TYPE] = LGeneralMethods::GetObjectTargetType(enteredNode_);
...
void LSpaceBuildings::HandlePlanetOpen(StringHash eventType, VariantMap& eventData)
{
