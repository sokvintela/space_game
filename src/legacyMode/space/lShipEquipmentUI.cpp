...
LShipEquipment::LShipEquipment(Context * context, Node* controlNode, UIElement* uiSpaceState)
	: Object(context)
...
void LShipEquipment::CreateShipInfo()
{
	...
	SubscribeToEvent(E_DRAGBEGIN, URHO3D_HANDLER(LShipEquipment, HandleDragBegin));
	SubscribeToEvent(E_DRAGMOVE, URHO3D_HANDLER(LShipEquipment, HandleDragMove));
	SubscribeToEvent(E_DRAGEND, URHO3D_HANDLER(LShipEquipment, HandleDragEnd));

	SubscribeToEvent(E_NODEADDED, URHO3D_HANDLER(LShipEquipment, HandleNodeAdded));
}

void LShipEquipment::HandleShipInfoUpdate()
{
	LEquipmentManager* eManager = controlNode_->GetComponent<LEquipmentManager>();
	if (!eManager)
		return;
	...
	for (U32 i = 0; i < equipmentMaxSize; i++)
	{
		...
		Texture2D* equipmentTexture = LGeneralMethods::GetTextureFromNode(equipmentChildren[i]);
		if (equipmentTexture)
		{
		...
		if (equipmentChildren[i]->GetComponent<CrystalDetector>())
			SubscribeToEvent(equipmentButton, E_PRESSED, URHO3D_HANDLER(LShipEquipment, HandleCrystalDetectorClick));
	}
...
*/
	...
	for (U32 i = 0; i <= cargoChildren.Size(); i++)
	{
		...
		*/
		...
		Texture2D* cargoTexture = LGeneralMethods::GetTextureFromNode(equipmentChildren[i]);
		if (cargoTexture)
		{
...
void LShipEquipment::HandleDragBegin(StringHash eventType, VariantMap& eventData)
{
...
void LShipEquipment::HandleDragMove(StringHash eventType, VariantMap& eventData)
{
...
void LShipEquipment::HandleDragEnd(StringHash eventType, VariantMap& eventData)
{
	LEquipmentManager* eManager = controlNode_->GetComponent<LEquipmentManager>();
	if (!eManager)
		return;
...
void LShipEquipment::HandleNodeAdded(StringHash eventType, VariantMap & eventData)
{
	HandleShipInfoUpdate();
}

bool LShipEquipment::IsPlacementCorrect(Modules::Module moduleType, Node* node) const
{
...
void LShipEquipment::HandleCrystalDetectorClick(StringHash eventType, VariantMap & eventData)
{
	VariantMap crystalDetectorUseEventData;
	crystalDetectorUseEventData[CrystalDetectorUseEvent::PLAYER_ID] = controlNode_->GetID();
	GetSubsystem<LSharedData>()->SendGameEvent(RE_DETECTOR_USE, crystalDetectorUseEventData);
}
