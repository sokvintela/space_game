...
void SGameShipInfoState::RegisterEvents(Context * context)
{
	Network* network = context->GetSubsystem<Network>();
	network->RegisterRemoteEvent(RE_EQUIPMENT_MOVE);
}

SGameShipInfoState::SGameShipInfoState(Urho3D::Context * context, UIElement* uiSubState) :
	ISubState(context, uiSubState)
{
}

void SGameShipInfoState::Enter(Node* playerNode)
{
	playerID_ = playerNode->GetID();
	equipmentManager_ = playerNode->GetComponent<TEquipmentManager>();
	...
	SubscribeToEvent(E_DRAGBEGIN, URHO3D_HANDLER(SGameShipInfoState, HandleDragBegin));
	SubscribeToEvent(E_DRAGMOVE, URHO3D_HANDLER(SGameShipInfoState, HandleDragMove));
	SubscribeToEvent(E_DRAGEND, URHO3D_HANDLER(SGameShipInfoState, HandleDragEnd));

	SubscribeToEvent(E_NODEADDED, URHO3D_HANDLER(SGameShipInfoState, HandleNodeAdded));

	SubscribeToEvent(backToSpace_, E_PRESSED, URHO3D_HANDLER(SGameShipInfoState, HandleBackToGameClick));
}

void SGameShipInfoState::Create(Node* targetNode)
{
...
void SGameShipInfoState::HandleDragBegin(StringHash eventType, VariantMap& eventData)
{
...
void SGameShipInfoState::HandleDragMove(StringHash eventType, VariantMap& eventData)
{
...
void SGameShipInfoState::HandleDragEnd(StringHash eventType, VariantMap& eventData)
{
...
void SGameShipInfoState::HandleBackToGameClick(StringHash eventType, VariantMap & eventData)
{
	Exit();
	parentState_->Resume();
}

void SGameShipInfoState::HandleNodeAdded(StringHash eventType, VariantMap & eventData)
{
	HandleShipInfoUpdate();
}

void SGameShipInfoState::HandleShipInfoUpdate()
{
