...
//#include "tacticalMode/buildings/shipFactory.h"
//#include "tacticalMode/equipment/hyperDrive.h"
...
//#include "tacticalMode/equipment/tRepairDrone.h"
#include "utility/sharedData.h"
#include "tacticalMode/spaceship/tDurability.h"
#include "tacticalMode/managers/tDataManager.h"
//#include "tacticalMode/galaxy/spaceObjects/tPlanet.h"
#include "utility/generalMethods.h"
//#include "tacticalMode/buildings/tBuilding.h"
#include "tacticalMode/playerLogic/tTeam.h"

#include "strategicMode/SubStates/sSpacePlanetState.h"

String sBuildingName = "buildingName";
String sResource = "resource";
String sEquipmentType = "equipmentType";
String sEquipmentName = "equipmentName";
String sTargetType = "targetType";

using namespace Urho3D;

void SGameSpacePlanetState::RegisterEvents(Context * context)
{
	/*
...
SGameSpacePlanetState::SGameSpacePlanetState(Urho3D::Context * context, UIElement* uiSubState) :
	ISubState(context, uiSubState),
	isCargo_(false)
{
}

void SGameSpacePlanetState::Enter(Node* planetNode)
{
	...
	TPlanet* planet = planetNode_->GetComponent<TPlanet>();
	if (planet) // && planet->GetPlanetType() == TPlanet::PlanetType::PT_Solid)
	{
		colonizedText_->SetText("Colonized");

		TDataManager* dManager = planetNode_->GetComponent<TDataManager>();
		if (dManager)
	...
	planetImage_->SetTexture(GeneralMethods::GetTextureFromNode(planetNode_));

	//buildingManager_ = planetNode_->GetComponent<BuildingsManager>();
	equipmentManager_ = controlNode_->GetComponent<TEquipmentManager>();
	...
	// buildings
	SubscribeToEvent(buildChoicesList_, E_ITEMSELECTED, URHO3D_HANDLER(SGameSpacePlanetState, HandleBuildItemSelected));
	SubscribeToEvent(completedBuildingsList_, E_ITEMSELECTED, URHO3D_HANDLER(SGameSpacePlanetState, HandleOpenBuildingInfo));

	// building
	SubscribeToEvent(improveBuildingButton_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleImproveBuilding));
	SubscribeToEvent(getResourceButton_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleTakeBuildingResourceDialog));

	// clouses
	SubscribeToEvent(closeBuildingButton_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleCloseBuilding));

	// update
	Scene* scene = parentState_->GetScene();
	if (scene)
		SubscribeToEvent(scene, E_SCENEUPDATE, URHO3D_HANDLER(SGameSpacePlanetState, HandleUpdate));

	SubscribeToEvent(backToSpace_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleBackToGameClick));
	SubscribeToEvent(showCargo_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleShowCargoClick));
}

void SGameSpacePlanetState::Create(Node* playerNode)
{
...
void SGameSpacePlanetState::HandleBuildItemSelected(StringHash eventType, VariantMap & eventData)
{
	...
	GetSubsystem<SharedData>()->SendGameEvent(RE_OBJECT_BUILD, buildEventData);
	*/
}

void SGameSpacePlanetState::HandleOpenBuildingInfo(StringHash eventType, VariantMap & eventData)
{
	U32 index = eventData[ItemSelected::P_SELECTION].GetUInt();
	UIElement* building = completedBuildingsList_->GetItem(index);
	String bName = building->GetVar(sBuildingName).GetString();
	...
	buildingInfoBar_->SetVar(sBuildingName, bName);

	buildingItemsList_->RemoveAllItems();
	if (bName == "radar")
	{
		getResourceButton_->SetVisible(true);
		getResourceButton_->SetVar(sResource, LGameResource::ResourceType::Energy);
		improveBuildingButton_->SetVisible(true);
	}
	else if (bName == "metalExtractionFactory")
	{
		getResourceButton_->SetVisible(true);
		getResourceButton_->SetVar(sResource, LGameResource::ResourceType::Metals);
	}
	else if (bName == "probesFactory")
	{
		...
		//GeneralMethods::ShowCost(createProbeButton, controlNode_, cost.first_, cost.second_);

		buildingItemsList_->AddItem(probesBar);

		SubscribeToEvent(createProbeButton, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleCreateProbe));
	}
	...
	else if (bName == "shipsFactory")
	{
		...
		for (U32 i = 0; i < Mob::MobType::MT_Count; i++)
		{
			...
			SubscribeToEvent(createShipButton_[i], E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleCreateShipClick));
		}
	}
	else if (bName == "orbitalFactory")
	{
		...
		for (U32 i = 0; i < size; i++)
		{
			...
			createOrbitalBuildingButton_[i]->SetVar(sBuildingName, "defenderPlatform");
			...
			//GeneralMethods::ShowCost(createOrbitalBuildingButton_[i], controlNode_, cost.first_, cost.second_);

			buildingItemsList_->AddItem(orbitalBar);
			SubscribeToEvent(createOrbitalBuildingButton_[count], E_PRESSED,
				URHO3D_HANDLER(SGameSpacePlanetState, HandleCreateOrbitalBuilding));
	...
	else if (bName == "repairFactory")
	{
		...
		SubscribeToEvent(repairButton_, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleRepair));
	}
}

void SGameSpacePlanetState::HandleTakeBuildingResourceDialog(StringHash eventType, VariantMap & eventData)
{
	...
	GetSubsystem<SharedData>()->SendGameEvent(RE_GET_BUILDING_RESOURCE, takeResourceEventData);
	*/
}

void SGameSpacePlanetState::HandleImproveBuilding(StringHash eventType, VariantMap & eventData)
{
	/*
	...
	GetSubsystem<SharedData>()->SendGameEvent(RE_IMPROVE_BUILDING, improveBuildingEventData);
	*/
}

void SGameSpacePlanetState::HandleCreateProbe(StringHash eventType, VariantMap & eventData)
{
	/*
	VariantMap createProbeEventData;
	createProbeEventData[CreateProbeEvent::PLAYER_ID] = controlNode_->GetID();

	GetSubsystem<SharedData>()->SendGameEvent(RE_CREATE_PROBE, createProbeEventData);
	*/
}

void SGameSpacePlanetState::HandleCreateEquipment(StringHash eventType, VariantMap & eventData)
{
	Button* selectedButton = static_cast<Button*>(eventData[Pressed::P_ELEMENT].GetPtr());
	/*
	...
	GetSubsystem<SharedData>()->SendGameEvent(RE_CREATE_EQUIPMENT, createEquipmentEventData);
	*/
}

void SGameSpacePlanetState::HandleCreateShipClick(StringHash eventType, VariantMap & eventData)
{
	// TODO: ptrs
	/*
	Button* selectedButton = static_cast<Button*>(eventData[Pressed::P_ELEMENT].GetPtr());
	for (U32 i = 0; i < createShipButton_.Size(); i++)
		if (createShipButton_[i] == selectedButton)
		{
			...
			GetSubsystem<SharedData>()->SendGameEvent(RE_MOBS_PRODUCTION, productionMobsEventData);
		}
		*/
}

void SGameSpacePlanetState::HandleCreateOrbitalBuilding(StringHash eventType, VariantMap & eventData)
{
	/*
	Button* selectedButton = static_cast<Button*>(eventData[Pressed::P_ELEMENT].GetPtr());
	...
	GetSubsystem<SharedData>()->SendGameEvent(RE_OBJECT_BUILD, createBuildingEventData);
	*/
}

void SGameSpacePlanetState::HandleRepair(StringHash eventType, VariantMap & eventData)
{
	/*
	VariantMap RepairShipEventData;
	RepairShipEventData[RepairShipEvent::PLAYER_ID] = controlNode_->GetID();

	GetSubsystem<SharedData>()->SendGameEvent(RE_SHIP_REPAIR, RepairShipEventData);
	*/
}

void SGameSpacePlanetState::HandleScrap(StringHash eventType, VariantMap & eventData)
{
	// TODO: ptrs
	/*
	Button* selectedButton = static_cast<Button*>(eventData[Pressed::P_ELEMENT].GetPtr());
	for (U32 i = 0; i < scrapButton_.Size(); i++)
		if (scrapButton_[i] == selectedButton)
		{
			...
			GetSubsystem<SharedData>()->SendGameEvent(RE_PUT_ON_SCRAP, scrapEventData);
		}
		*/
}

void SGameSpacePlanetState::HandleCloseBuilding(StringHash eventType, VariantMap & eventData)
{
	buildingInfoBar_->SetVisible(false);
	constructedBar_->SetVisible(true);
}

void SGameSpacePlanetState::HandleBackToGameClick(StringHash eventType, VariantMap & eventData)
{
	Exit();
	parentState_->ExitSubstate();
}

void SGameSpacePlanetState::HandleShowCargoClick(StringHash eventType, VariantMap & eventData)
{
	cargoElement_->SetVisible(!cargoElement_->IsVisible());
}

void SGameSpacePlanetState::Exit()
{
	uiSubStateRoot_->SetVisible(false);
...
void SGameSpacePlanetState::UpdateCompleteBuildings()
{
	/*
	...
	for (auto building : buildings)
	{
		...
		buildingImage->SetTexture(GeneralMethods::GetTextureFromNode(building));
...
void SGameSpacePlanetState::ShowEquipment(LEquipment::EquipmentType eType)
{
	...
	for (auto& equipmentIt : availabeEquipment)
	{
		...
		Texture2D* texture = GeneralMethods::GetTextureFromNode(equipmentIt.second_);
		if (texture)
		...
		createEquipmentButton->SetVar(sEquipmentType, eType);
		createEquipmentButton->SetVar(sEquipmentName, equipmentIt.first_);

		auto cost = prefabs->GetLegacyEquipmentCreateCost(eType, equipmentIt.first_);
		//GeneralMethods::ShowCost(createEquipmentButton, controlNode_,
			//cost.first_, cost.second_);

		buildingItemsList_->AddItem(equipmentBar);
		SubscribeToEvent(createEquipmentButton, E_PRESSED, URHO3D_HANDLER(SGameSpacePlanetState, HandleCreateEquipment));
	}
}

void SGameSpacePlanetState::UpdateProposalBuildings()
{
	/*
	if (!buildingManager_)
		return;
	...
	for (auto bName : buildingsProposal)
	{
		...
		radioImage->SetTexture(GeneralMethods::GetTextureFromNode(buildingPrefabNode));
		radioImage->SetFixedSize(50, 30);
		radioImage->SetStyleAuto();

		// TODO: does not work
		GeneralMethods::ShowCost(radioImage, controlNode_, TGameResource::ResourceType::Metals, 10000.f);
...
void SGameSpacePlanetState::HandleUpdate(StringHash eventType, VariantMap & eventData)
{
	ResourceCache* cache = GetSubsystem<ResourceCache>();
	PrefabsStorage* prefabsStorage = GetSubsystem<PrefabsStorage>();
...
*/
	...
	*/
	...
	if (buildingInfoBar_->IsVisible())
	{
		...
		if (bName == "radar")
		{
			TResourceManager* rManager = planetNode_->GetComponent<TResourceManager>();
			Text* radarEnergy = static_cast<Text*>(buildingItemsList_->GetItem(0));
			if (radarEnergy)
			{
				radarEnergy->SetText(String(static_cast<U32>(rManager->GetResourceValue(TGameResource::ResourceType::Energy))));
			}
		}
		else if (bName == "metalExtractionFactory")
		{
			...
			if (metals)
			{
				metals->SetText(String(static_cast<U32>(rManager->GetResourceValue(TGameResource::ResourceType::Metals))));
			}
		}
		else if (bName == "weaponFactory")
		{
		...
		{
			TDurability* shipDurability = controlNode_->GetComponent<TDurability>();
			if (shipDurability)
			{
		...
		else if (bName == "scrapMetalFactory")
		{
			...
			for (U32 i = 0; i < children.Size(); i++)
			{
				...
				SubscribeToEvent(scrapButton_[i], E_PRESSED,
					URHO3D_HANDLER(SGameSpacePlanetState, HandleScrap));
			}
