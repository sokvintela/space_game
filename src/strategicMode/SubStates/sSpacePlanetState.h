...
class SGameSpacePlanetState : public ISubState
{
	URHO3D_OBJECT(SGameSpacePlanetState, ISubState);
public:

	static void RegisterEvents(Context * context);

	SGameSpacePlanetState(Urho3D::Context * context, UIElement * uiSubState);
	virtual ~SGameSpacePlanetState() = default;
...
private:
	...
	//WeakPtr<BuildingsManager> buildingManager_;
