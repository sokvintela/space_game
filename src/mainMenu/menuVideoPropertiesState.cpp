	...
	languageIndex_(0)
{
	...
	PODVector<IntVector2> supportedResolutions = graphics->GetResolutions();
	for (U32 i = 0; i < supportedResolutions.Size(); i++)
	{
		resolutions_.Push(Resolution(supportedResolutions[i].x_, supportedResolutions[i].y_));
	}
}
...
void MenuVideoPropertiesState::HandleApplyButtonClick(StringHash eventType, VariantMap & eventData)
{
	...
	graphics->SetMode(res.width, res.height, fullscreen, borderless, false, false, false, false, false);
